...
export type PageDocContentType =
  | {
      type: 'affine:paragraph';
      text: string;
      variant: 'text' | 'h1' | 'h2' | 'h3' | 'h4';
    }
  | {
      type: 'affine:list' | 'affine:code';
      text: string;
    }
  | {
      type: 'affine:image';
...
export interface PageDocContent {
  title: string;
  summary: string;
  // we lose the depth information. is it necessary?
  content: PageDocContentType[];
}
    ...
    switch (flavour) {
      ...
      case 'affine:code': {
        ...
        if (summaryLenNeeded > 0) {
          content.summary +=
            text.toString().substring(0, summaryLenNeeded) + ' ';
      ...
      case 'affine:image': {
        const blobId = block.get('prop:sourceId') as string;
        if (blobId) {
          content.content.push({
            type: flavour,
            blobId,
            alt: block.get('prop:caption') as string,
          });
        }
        break;
      }
